...
CREATE TABLE calisanlar 
 ...
 -- in yerine = olmaz cunku = bir veri icin yazilabilir
 ...
 /* ----------------------------------------------------------------------------
  ...
  select isim, maas, sehir, isyeri  from calisanlar -- esas sorgu burasi tabloda gozukenler
  where isyeri in(select marka_isim from markalar where marka_id>101); -- parantezin ici ikinci tabloyu calistirir
...
==============================================================================*/   
 ...
 -----------------------------------------------------------------------------*/
  ...
  select marka_isim, calisan_sayisi, (select SUM(maas) from calisanlar
										where markalar.marka_isim=calisanlar.isyeri) as toplam_maas -- as sart degil ama sutuna isim verirken kullanilir
  from markalar;
 ...
 -----------------------------------------------------------------------------*/
  ...
  select marka_isim, calisan_sayisi, (select round(AVG(maas)) from calisanlar  -- round sayiyi yuvarla -paranteze bitisik yaziyoruz -AVG ort. demek
 ...
 select marka_isim, calisan_sayisi, (select max(maas) from calisanlar
									where markalar.marka_isim=calisanlar.isyeri) max_maas,
                                    (select min(maas) from calisanlar
                                    where markalar.marka_isim=calisanlar.isyeri) min_maas
 from markalar;
